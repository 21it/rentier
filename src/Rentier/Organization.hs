...
schedule2Ranges thisWd xs =
  ...
  where
    onlyPosRules =
      filter (\x -> organizationScheduleTimeKind x == WorkingTime)
    onlyNegRules =
      filter (\x -> organizationScheduleTimeKind x /= WorkingTime)
    parseRange x =
      let MinutesRange from to =
            tod2minsRange
              (organizationScheduleStartsAt x)
              (organizationScheduleEndsAt x)
    ...
    nextNeg =
      map
        (reParseRange . minsRangeDayShift)
        (map parseRange (onlyNegRules nextXs) >>= spanRangeOnly)
