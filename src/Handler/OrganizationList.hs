...
getOrganizationListR merchantId = do
  ...
  list <- runDB $ selectList
    [OrganizationMerchantId P.==. merchantId, OrganizationExists P.==. True]
    [Desc OrganizationId]
  app <- getYesod
  langs <- languages
  let table =
        textCol (hr2Text MsgOrganizationName) (organizationName . entityVal) <>
        textCol (hr2Text MsgCurrencyCode) (Data.Text.pack . show . organizationBaseCurrencyCode . entityVal) <>
        textCol (hr2Text MsgDefaultMinimalRentDuration) (hr2Text . organizationDefaultMinimalRentDuration . entityVal) <>
        textColHiddenXs (hr2Text MsgDefaultIframeTimeResolution) (hr2Text . organizationDefaultIframeTimeResolution . entityVal) <>
        textColHiddenXs (hr2Text MsgActive) (hr2Text . organizationActive . entityVal) <>
...
simpleListSettings user merchantId =
  SimpleListSettings{
    listPageTitle = MsgOrganizationListRTitle $ userIdent user,
