...
data SessionConfirmForm = SessionConfirmForm{
  ...
  sessionConfirmPropertiesMultipleSelection :: Maybe PropertiesMultipleSelection,
  sessionConfirmCustomerComment             :: Maybe Textarea
} deriving Show

getIframeSessionConfirmR :: PropertyId -> UTC -> UTC -> PropertiesMultipleSelection -> Text -> Handler Html
getIframeSessionConfirmR propertyId (UTC startsAt) (UTC endsAt) (PropertiesMultipleSelection additionalProperties) customerComment =
  ...
  where
    ...
    renderer org mainProperty ps = do
      ...
      (formWidget, formEnctype) <- generateFormPost $
                                   ...
                                   (aForm org propertyId mainProperty startsAt endsAt ps additionalProperties customerComment hr2Text)
...
aForm org propertyId mainProperty startsAt endsAt ps additionalProperties customerComment hr2Text =
  ...
  <*> aopt (multipleSelectField selectable PropertiesMultipleSelection show) (bfsDisabled MsgProperty) (Just $ Just $ PropertiesMultipleSelection allProperties)
  <*> aopt textareaField (bfsDisabled MsgComment) (Just $ Just $ Textarea customerComment)
  where
    allProperties = (propertyId, 1) : additionalProperties
    startsTime = utcToLocalTime utc startsAt
    selectedProps = Data.List.foldl
      (\acc (k, v) -> Map.insert k v acc)
      Map.empty
      allProperties
    labelFormatter name qty =
      pack $ "[" ++ (unpack $ hr2Text $ MsgPcs qty) ++ "] " ++ (unpack name)
    selectable =
      (Entity{entityKey = propertyId, entityVal = mainProperty} : ps)
      >>= (\Entity{entityKey = k, entityVal = Property{propertyName = name}} ->
        case Map.lookup k selectedProps of
          Just 0   -> []
          Just qty -> [(labelFormatter name qty, k, [qty])]
          Nothing  -> []
      )
