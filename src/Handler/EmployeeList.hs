...
getEmployeeListR merchantId = do
  ...
  list <- runDB $ selectList
    [EmployeeMerchantId P.==. merchantId, EmployeeExists P.==. True]
    [Asc EmployeeId]
  app <- getYesod
  langs <- languages
  let table =
        textCol (hr2Text MsgEmployeeIdent) (employeeIdent . entityVal) <>
        textCol (hr2Text MsgEmployeeInvitationStatus) (hr2Text . employeeInvitationStatus . entityVal) <>
        textColHiddenXs (hr2Text MsgActive) (hr2Text . employeeActive . entityVal)
...
simpleListSettings user merchantId =
  SimpleListSettings{
    listPageTitle = MsgEmployeeListRTitle $ userIdent user,
